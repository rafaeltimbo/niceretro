...
describe PositiveTopicsController do
  ...
  let!(:positive_topic) { PositiveTopic.create(description: 'Initial description') }

  describe 'POST #create' do
    it 'return success status' do
      post :create, positive_topic: { description:'positive_topic' }, retrospective_id: retrospective.id, format: :js
  ...
  describe 'DELETE #destroy' do
    it 'returns success status' do
      delete :destroy, retrospective_id: retrospective.id, id: positive_topic.id, format: :js
      expect(response).to have_http_status(200)
    end
  ...
  describe 'POST #edit' do
    it 'return success status' do
      post :edit, retrospective_id: retrospective.id, id: positive_topic.id, format: :js
      expect(response).to have_http_status(200)
    end
  ...
  describe 'POST #update' do
    ...
    context 'when positive topic does not exist' do
      ...
      it 'return 404 error status' do
        put :update, retrospective_id: retrospective.id, id: positive_topic.id, format: :js
        expect(response).to have_http_status(404)
      end
